...
public class AdminController {
    ...
    public String addBoardSuccess(@RequestParam String title, @RequestParam String text, @RequestParam int userId) {
        ...
        board.setBoardName(new EncodingTool().encodeStr(title));
        board.setTopicNum(0);
        board.setBoardDesc(new EncodingTool().encodeStr(text));
        adminService.addBoard(board, userId);
        return "redirect:/set_board";
    }
    ...
    public String deleteArt(@RequestParam int topicId, @RequestParam int userId, @RequestParam int boardId) {
        ...
        return "redirect:/admin_board/" + boardId;
    }
    ...
    @RequestMapping(value = "/admin_article/{id}")
    @ApiOperation(value = "管理员根据id寻找主题帖", httpMethod = "GET")
    public ModelAndView adminTopicPage(@PathVariable("id") int topicId, @RequestParam(required = false, defaultValue = "1") Integer page) {
        int PageSize = 5;
        PageHelper.startPage(page, PageSize);
        List<Post> list = forumService.getAllPostReplyByTopicId(topicId);
        ModelAndView modelAndView = new ModelAndView();
        modelAndView.setViewName("admin_topic");
    ...
    @RequestMapping(value = "/deletePostByAdmin", method = RequestMethod.DELETE)
    @ApiOperation(value = "管理员删除回复贴", httpMethod = "DELETE")
    public String deletePostByAdmin(@RequestParam int postId, @RequestParam int topicId) {
        forumService.deletePost(postId);
        return "redirect:/admin_article/" + topicId;
    }
